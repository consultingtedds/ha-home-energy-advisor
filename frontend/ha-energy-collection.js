...
/**
 * Home Assistant's period picker, and the two tags that locate it.
 *
 * The picker decides which way its calendar opens through `popoverPlacement`,
 * a reactive property of `ha-date-range-picker`. `hui-energy-period-selector`
 * derives that from a memoised measurement: in Home Assistant's own energy
 * panel it settles on `top-start`, and in the `energy-date-selection` card
 * inside a view footer it settles on `bottom-start` - which draws a 553 px
 * calendar downward from a control already at the bottom of the viewport, where
 * a sticky footer never scrolls to reach it (HEA-115).
 */
const PICKER_TAG = "ha-date-range-picker";
const FOOTER_TAG = "hui-view-footer";
const ABOVE = "top-start";

/** The view footer enclosing an element, crossing shadow boundaries. */
const enclosingFooter = (from) => {
  let node = from;
  while (node) {
    if (node.tagName?.toLowerCase() === FOOTER_TAG) return node;
    node = node.parentElement ?? node.getRootNode?.()?.host ?? null;
  }
  return null;
};

/** The first picker beneath a root, crossing shadow boundaries. */
const pickerWithin = (root, depth = 0) => {
  if (depth > 8) return null;
  for (const element of root.querySelectorAll("*")) {
    if (element.tagName.toLowerCase() === PICKER_TAG) return element;
    const nested = element.shadowRoot && pickerWithin(element.shadowRoot, depth + 1);
...
/**
 * Point the period picker's calendar upward, and say whether one was found.
 *
 * Scoped to the view footer holding ``from``, which must be one of our own
 * elements. This bundle loads on every page, so reaching for any picker in the
 * document would change Home Assistant's own dashboards as well as ours.
 *
 * Returns ``false`` when the footer or the picker is not there yet, so a caller
 * can try again rather than assume a picker it never found is now pointing the
 * right way. Setting the property is idempotent and safe to repeat: the
 * selector owns it and recomputes it, so re-applying is the only way to stay
 * pointed the right way.
 */
export const pinPeriodPickerAbove = (from) => {
  const footer = enclosingFooter(from);
  const picker = footer && pickerWithin(footer);
  if (!picker) return false;
  picker.popoverPlacement = ABOVE;
  return true;
};
