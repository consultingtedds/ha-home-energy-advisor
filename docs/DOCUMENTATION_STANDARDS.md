...
- Punctuate with characters that are on a keyboard. Use a plain hyphen `-` for
  every dash, in prose, comments, commit messages and user-facing strings alike;
  no em dash (U+2014) and no en dash (U+2013), including in numeric ranges.
  Anyone editing this repo should be able to type what they read
