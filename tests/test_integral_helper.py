...
Power-only devices (a cloud lamp wall lights, a cloud heating integration ``effective_power``) carry no energy
...
2. it accrues **no phantom energy** across an ``unavailable`` span — the
   "a cloud heating integration unplugged for six months" case — which is exactly the reset-on-
...
async def test_a_native_integral_helper_integrates_power_to_energy(
    ...
    entry_id = await async_ensure_integral_helper(
        hass,
        name="Slow Poll Wall Lights Energy",
...
async def test_no_phantom_energy_accrues_across_an_unavailable_span(
    ...
    freezer.move_to(datetime(2026, 7, 8, 22, 0, tzinfo=UTC))
    hass.states.async_set("sensor.power_only_panel_heater_power", "100", _POWER)
    entry_id = await async_ensure_integral_helper(
        hass,
        name="Power Only Panel Heater Energy",
        source_entity="sensor.power_only_panel_heater_power",
    )
    await hass.async_block_till_done()
    output = integral_output_sensor(hass, entry_id)
    assert output is not None
    ...
    hass.states.async_set("sensor.power_only_panel_heater_power", "unavailable")
    await hass.async_block_till_done()
