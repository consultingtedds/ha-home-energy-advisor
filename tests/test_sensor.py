...
async def test_reset_leaves_the_split_reconciling_as_it_accumulates_again(
    ...
    guest = energy("sensor.guest_bedroom_aircon_energy_used")
...
async def test_each_device_gets_energy_by_source_sensors(
    ...
    assert energy("sensor.guest_bedroom_aircon_energy_from_grid") == Decimal("0.285714")
    assert energy("sensor.guest_bedroom_aircon_energy_from_generation") == Decimal(
        "0.214286"
    )
    assert energy("sensor.guest_bedroom_aircon_energy_from_battery") == Decimal(0)

    # And — the three sum to the device's energy, which is what makes a
    # self-sufficiency percentage total 100 %
    total = sum(
        energy(f"sensor.guest_bedroom_aircon_{concept}") for concept in _BY_SOURCE
    )
    assert total == energy("sensor.guest_bedroom_aircon_energy_used")
...
async def test_untracked_by_source_is_total_because_late_energy_pulls_it_down(
    ...
    for concept in _BY_SOURCE:
        state = hass.states.get(f"sensor.guest_bedroom_aircon_{concept}")
        assert state is not None
...
async def test_devices_sensor_exposes_the_source_devices_area_and_floor(
    ...
    _place_source_in_an_area(
        hass, entity_id="sensor.guest_energy", area="Guest Bedroom", floor="First Floor"
    )
    _seed_states(hass)
    entry = _entry()
    entry.add_to_hass(hass)
    ...
    guest = by_key["guest_bedroom_aircon"]
    assert guest["area_name"] == "Guest Bedroom"
...
async def test_an_area_on_the_source_entity_wins_over_its_devices(
    ...
    freezer.move_to(datetime(2026, 7, 8, 22, 0, tzinfo=UTC))
    _place_source_in_an_area(
        hass, entity_id="sensor.guest_energy", area="Guest Bedroom"
    )
    _seed_states(hass)
    ...
    by_key = await _devices_payload(hass, entry, freezer)
    assert by_key["guest_bedroom_aircon"]["area_name"] == "Landing"
...
async def test_hierarchy_is_exposed_without_touching_heas_own_devices(
    ...
    assert hass.states.get("sensor.guest_bedroom_aircon_energy_used") is not None
