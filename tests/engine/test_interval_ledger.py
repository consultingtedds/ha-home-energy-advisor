...
def test_spread_energy_across_a_long_outage_conserves_and_spans_every_bucket() -> None:
    # Given — the tumble dryer's counter jumps 50 kWh over a three-day outage
...
def test_ledger_keeps_device_draws_separate_from_each_other() -> None:
    ...
    assert draws["coarse_step_aircon"] == Decimal("0.25")
    assert draws["fine_meter_aircon"] == Decimal("0.15")
...
def test_ledger_spreads_a_device_delta_across_the_buckets_it_spans() -> None:
    ...
    ledger = IntervalLedger()
    ledger.add_device(
        "coarse_step_aircon", delta(start="02:14", end="02:19", kwh="0.25")
    )
    ...
    assert buckets[0].device_draws["coarse_step_aircon"] == Decimal("0.05")
    assert buckets[1].device_draws["coarse_step_aircon"] == Decimal("0.20")
